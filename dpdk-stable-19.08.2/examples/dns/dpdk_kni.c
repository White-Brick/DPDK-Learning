...
static void port_init(struct rte_mempool *mbuf_pool) {
	...
	struct rte_eth_conf local_port_conf = port_conf_default;	/** 如何初始化结构体 */
	
	rte_eth_dev_info_get(g_dpdkPortId, &dev_info);				/** 获取接口信息 */
	if (dev_info.tx_offload_capa & DEV_TX_OFFLOAD_MBUF_FAST_FREE) {	/** 接口硬件能力是否支持 */
	...
	if (rte_eth_macaddr_get(gDpdkPortId, &addr) <0 ) {
		rte_exit(EXIT_FAILURE, "Get MAC failed.\n")
	}
