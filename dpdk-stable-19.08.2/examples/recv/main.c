...
main(int argc, char * argv []) {
    ...
    port_init(gDpdkPortId, mbuf_pool);
#if 0
    ...
    /** 开始收包 */
    while (1) {
        uint16_t nb_rx = rte_eth_rx_burst(0, 0, rx_pkt, BURST_SIZE);
        if (nb_rx == 0) {
            continue;
        }
        unsigned i = 0;
...
//        struct rte_udp_hdr   *udp_hdr;
        ...
        for (i = 0; i < nb_rx; ++nb_rx) {
            ...
            ip_hdr = rte_pktmbuf_mtod(rx_pkt[i], struct rte_ipv4_hdr *);
